...
export class LearningSessionService {
  ...
  public async createReviewSession(archivedSession: boolean, collection?: number): Promise<SessionCard[]> {
    const settings = await this.settingsService.getSettings();
    const cards = archivedSession
      ? await this.collectionService.getArchivedCardRequest(collection).sortBy("leitnerBox")
      : await this.collectionService.getReviewCardRequest(collection).sortBy("leitnerBox");
